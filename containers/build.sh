...
if [[ -n $GITHUB_REF_NAME ]]; then
  ...
  AZM_AI_BUILD_VERSION=$sanitized_ref_name
...
if [[ $dry_run -eq 1 ]]; then
  ...
  jq -n \
    ...
    --arg azm_ai_build_version "$AZM_AI_BUILD_VERSION" \
    --arg dockerfile "$dir/Dockerfile" \
    '{
      tags: $tags,
      platform: $platform,
      build_args: [
        "AZM_AI_BUILD_VERSION=" + $azm_ai_build_version
      ],
...
docker buildx build \
  $args \
  --build-arg AZM_AI_BUILD_VERSION="$AZM_AI_BUILD_VERSION" \
