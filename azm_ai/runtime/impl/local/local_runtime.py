...
from azm_ai.events.serialization import event_to_dict, observation_from_dict
from azm_ai.runtime.impl.action_execution.action_execution_client import (
...
from azm_ai.runtime.utils import find_available_tcp_port
from azm_ai.runtime.utils.command import get_action_execution_server_startup_command
...
def check_dependencies(code_repo_path: str, poetry_venvs_path: str):
    ERROR_MESSAGE = 'Please follow the instructions in https://github.com/All-Hands-AI/AZM AI/blob/main/Development.md to install AZM AI.'
...
class LocalRuntime(ActionExecutionClient):
    ...
    def __init__(
        ...
        if self.config.workspace_base is not None:
            ...
            self._temp_workspace = tempfile.mkdtemp(
                prefix=f'azm_ai_workspace_{sid}',
            )
            self.config.workspace_mount_path_in_sandbox = self._temp_workspace

        logger.warning(
            'Initializing LocalRuntime. WARNING: NO SANDBOX IS USED. '
            'This is an experimental feature, please report issues to https://github.com/All-Hands-AI/AZM AI/issues. '
            '`run_as_azm_ai` will be ignored since the current user will be used to launch the server. '
    ...
    async def connect(self):
        ...
        code_repo_path = os.path.dirname(os.path.dirname(azm_ai.__file__))
