...
from azm_ai.events.action import Action, IPythonRunCellAction
from azm_ai.events.observation import IPythonRunCellObservation
from azm_ai.runtime.plugins.jupyter.execute_server import JupyterKernel
...
class JupyterPlugin(Plugin):
    ...
    async def initialize(
        self, username: str, kernel_id: str = 'azm-ai-default'
    ) -> None:
        self.kernel_gateway_port = find_available_tcp_port(40000, 49999)
        self.kernel_id = kernel_id
        if username in ['root', 'azm_ai']:
            # Non-LocalRuntime
            prefix = f'su - {username} -s '
            # cd to code repo, setup all env vars and run micromamba
            poetry_prefix = (
                'cd /azm_ai/code\n'
                'export POETRY_VIRTUALENVS_PATH=/azm_ai/poetry;\n'
                'export PYTHONPATH=/azm_ai/code:$PYTHONPATH;\n'
                'export MAMBA_ROOT_PREFIX=/azm_ai/micromamba;\n'
                '/azm_ai/micromamba/bin/micromamba run -n azm_ai '
            )
        else:
            # LocalRuntime
            prefix = ''
            code_repo_path = os.environ.get('AZM_AI_REPO_PATH')
            if not code_repo_path:
                raise ValueError(
                    'AZM_AI_REPO_PATH environment variable is not set. '
