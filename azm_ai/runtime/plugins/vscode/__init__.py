...
from azm_ai.runtime.utils.system import check_port_available
...
class VSCodePlugin(Plugin):
    ...
    async def initialize(self, username: str) -> None:
        if username not in ['root', 'azm_ai']:
            self.vscode_port = None
            self.vscode_connection_token = None
            logger.warning(
                'VSCodePlugin is only supported for root or azm_ai user. '
        ...
        cmd = (
            ...
            f'sudo chown -R {username}:{username} /azm_ai/.openvscode-server\n'
            'cd /workspace\n'
            f'exec /azm_ai/.openvscode-server/bin/openvscode-server --host 0.0.0.0 --connection-token {self.vscode_connection_token} --port {self.vscode_port} --disable-workspace-trust\n'
            'EOF'
        )
