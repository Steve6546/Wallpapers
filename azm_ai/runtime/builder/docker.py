...
from azm_ai.runtime.builder.base import RuntimeBuilder
from azm_ai.utils.term_color import TermColor, colorize
...
class DockerRuntimeBuilder(RuntimeBuilder):
    ...
    def build(
        ...
        if not DockerRuntimeBuilder.check_buildx(self.is_podman):
            # when running azm_ai in a container, there might not be a "docker"
            # binary available, in which case we need to download docker binary.
            # since the official azm_ai app image is built from debian, we use
            # debian way to install docker binary
            logger.info(
                'No docker binary available inside azm-ai-app container, trying to download online...'
            )
            commands = [
                'apt-get update',
