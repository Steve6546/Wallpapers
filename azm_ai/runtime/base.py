...
from azm_ai.core.config import AppConfig, SandboxConfig
from azm_ai.core.exceptions import AgentRuntimeDisconnectedError
from azm_ai.core.logger import azm_ai_logger as logger
from azm_ai.events import EventSource, EventStream, EventStreamSubscriber
...
from azm_ai.events.serialization.action import ACTION_TYPE_TO_CLASS
from azm_ai.integrations.provider import (
    PROVIDER_TOKEN_TYPE,
    ProviderHandler,
    ProviderType,
)
from azm_ai.integrations.service_types import Repository
from azm_ai.microagent import (
    BaseMicroagent,
    load_microagents_from_dir,
)
from azm_ai.runtime.plugins import (
    JupyterRequirement,
    PluginRequirement,
    VSCodeRequirement,
)
from azm_ai.runtime.utils.edit import FileEditRuntimeMixin
from azm_ai.runtime.utils.git_handler import CommandResult, GitHandler
from azm_ai.utils.async_utils import (
    GENERAL_TIMEOUT,
...
class Runtime(FileEditRuntimeMixin):
    ...
    async def clone_repo(
        ...
        azm_ai_workspace_branch = f'azm-ai-workspace-{random_str}'
        ...
        checkout_command = (
            ...
            if selected_branch
            else f'git checkout -b {azm_ai_workspace_branch}'
        )
    ...
    def maybe_run_setup_script(self):
        """Run .azm_ai/setup.sh if it exists in the workspace or repository."""
        setup_script = '.azm_ai/setup.sh'
    ...
    def get_microagents_from_selected_repo(
        ...
        microagents_dir = workspace_root / '.azm_ai' / 'microagents'
        repo_root = None
        if selected_repository:
            repo_root = workspace_root / selected_repository.split('/')[-1]
            microagents_dir = repo_root / '.azm_ai' / 'microagents'
        self.log(
            'info',
        ...
        # Check for legacy .azm_ai_instructions file
        obs = self.read(
            FileReadAction(path=str(workspace_root / '.azm_ai_instructions'))
        )
        if isinstance(obs, ErrorObservation) and repo_root is not None:
            ...
            self.log(
                'debug',
                f'.azm_ai_instructions not present, trying to load from repository {microagents_dir=}',
            )
            obs = self.read(
                FileReadAction(path=str(repo_root / '.azm_ai_instructions'))
            )

        if isinstance(obs, FileReadObservation):
            self.log('info', 'azm_ai_instructions microagent loaded.')
            loaded_microagents.append(
                BaseMicroagent.load(
                    path='.azm_ai_instructions', file_content=obs.content
                )
            )
