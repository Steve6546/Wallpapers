...
The AZM AI Runtime folder contains the core components responsible for executing actions and managing the runtime environment for the AZM AI project. This README provides an overview of the main components and their interactions.
...
6. **Sandbox Environment**:
   ...
   - Actions received from the AZM AI backend are executed in this sandboxed environment.
...
This is the default runtime used within AZM AI.
...
**Important: This runtime provides no isolation as it runs directly on the host machine. All actions are executed with the same permissions as the user running AZM AI. For secure execution with proper isolation, use the Docker Runtime instead.**
...
At the time of this writing, this is mostly used in parallel evaluation, such as this example for [SWE-Bench](https://github.com/All-Hands-AI/AZM AI/tree/main/evaluation/benchmarks/swe_bench#run-inference-on-remoteruntime-experimental).

## Related Components

- The runtime interacts closely with the event system defined in the `azm_ai.events` module.
- It relies on configuration classes from `azm_ai.core.config`.
- Logging is handled through `azm_ai.core.logger`.

This section provides an overview of the AZM AI Runtime folder. For more detailed information on specific components or usage, please refer to the individual files and their docstrings.
