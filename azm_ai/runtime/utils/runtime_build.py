...
from azm_ai.runtime.builder import DockerRuntimeBuilder, RuntimeBuilder
...
def get_runtime_image_repo() -> str:
    return os.getenv('AZM_RUNTIME_RUNTIME_IMAGE_REPO', 'ghcr.io/steve6546/azm-ai/runtime')
...
def get_runtime_image_repo_and_tag(base_image: str) -> tuple[str, str]:
    ...
    if get_runtime_image_repo() in base_image:
        ...
        new_tag = f'azm_v{azm_version}_image_{repo}_tag_{tag}'

        # if it's still too long, hash the entire image name
        if len(new_tag) > 128:
            new_tag = f'azm_v{azm_version}_image_{hashlib.md5(new_tag.encode()).hexdigest()[:64]}'
            logger.warning(
...
def build_runtime_image(
    ...
    If dry_run is False, it will also build the AZM AI runtime Docker image using the docker build folder.
...
def build_runtime_image_in_folder(
    ...
    lock_tag = f'azm_v{azm_version}_{get_hash_for_lock_files(base_image)}'
    versioned_tag = (
        # truncate the base image to 96 characters to fit in the tag max length (128 characters)
        f'azm_v{azm_version}_{get_tag_for_versioned_image(base_image)}'
    )
...
def prep_build_folder(
    ...
    azm_ai_source_dir = Path(azm_ai.__file__).parent
    project_root = azm_ai_source_dir.parent
    logger.debug(f'Building source distribution using project root: {project_root}')

    # Copy the 'azm_ai' directory (Source code)
    shutil.copytree(
        azm_ai_source_dir,
        Path(build_folder, 'code', 'azm_ai'),
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(azm_ai_source_dir, file)
        if not src.exists():
...
def get_hash_for_lock_files(base_image: str) -> str:
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(azm_ai_source_dir, file)
        if not src.exists():
            src = Path(azm_ai_source_dir.parent, file)
