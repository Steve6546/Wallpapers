...
DEFAULT_PYTHON_PREFIX = [
    '/azm_ai/micromamba/bin/micromamba',
    'run',
    '-n',
    'azm_ai',
    'poetry',
...
def get_action_execution_server_startup_command(
    ...
    username = override_username or (
        'azm_ai' if app_config.run_as_azm_ai else 'root'
    )
    user_id = override_user_id or (
        sandbox_config.user_id if app_config.run_as_azm_ai else 0
    )
    ...
    base_cmd = [
        *python_prefix,
        ...
        '-u',
        '-m',
        'azm_ai.runtime.action_execution_server',
        str(server_port),
        '--working-dir',
