...
It is responsible for executing actions received from AZM AI backend and producing observations.
...
from azm_ai_aci.editor.editor import OHEditor
from azm_ai_aci.editor.exceptions import ToolError
from azm_ai_aci.editor.results import ToolResult
from azm_ai_aci.utils.diff import get_diff
from pydantic import BaseModel
from starlette.background import BackgroundTask
from starlette.exceptions import HTTPException as StarletteHTTPException
from uvicorn import run

from azm_ai.core.exceptions import BrowserUnavailableException
...
from azm_ai.events.serialization import event_from_dict, event_to_dict
from azm_ai.runtime.browser import browse
from azm_ai.runtime.browser.browser_env import BrowserEnv
from azm_ai.runtime.plugins import ALL_PLUGINS, JupyterPlugin, Plugin, VSCodePlugin
from azm_ai.runtime.utils.async_bash import AsyncBashSession
from azm_ai.runtime.utils.bash import BashSession
from azm_ai.runtime.utils.file_viewer import generate_file_viewer_html
from azm_ai.runtime.utils.files import insert_lines, read_lines
from azm_ai.runtime.utils.memory_monitor import MemoryMonitor
from azm_ai.runtime.utils.runtime_init import init_user_and_working_directory
from azm_ai.runtime.utils.system_stats import get_system_stats
from azm_ai.utils.async_utils import call_sync_from_async, wait_all
...
class ActionExecutor:
    ...
    async def ainit(self):
        ...
        if 'agent_skills' in self.plugins and 'jupyter' in self.plugins:
            obs = await self.run_ipython(
                IPythonRunCellAction(
                    code='from azm_ai.runtime.plugins.agent_skills.agentskills import *\n'
                )
            )
    ...
    async def _init_bash_commands(self):
        INIT_COMMANDS = [
            'git config --file ./.git_config user.name "azm_ai" && git config --file ./.git_config user.email "azm-ai@steve6546.com" && alias git="git --no-pager" && export GIT_CONFIG=$(pwd)/.git_config'
            if os.environ.get('LOCAL_RUNTIME_MODE') == '1'
            else 'git config --global user.name "azm_ai" && git config --global user.email "azm-ai@steve6546.com" && alias git="git --no-pager"'
        ]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--username', type=str, help='User to run as', default='azm_ai'
    )
