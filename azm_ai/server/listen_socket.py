...
from azm_ai.events.async_event_store_wrapper import AsyncEventStoreWrapper
...
from azm_ai.integrations.provider import PROVIDER_TOKEN_TYPE, ProviderToken
...
from azm_ai.storage.conversation.conversation_validator import (
...
@sio.event
async def azm_user_action(connection_id: str, data: dict):
    await conversation_manager.send_to_event_stream(connection_id, data)


@sio.event
async def azm_action(connection_id: str, data: dict):
    # TODO: Remove this handler once all clients are updated to use azm_user_action
