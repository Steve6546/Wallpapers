...
from azm_ai.core.config import AgentConfig, AppConfig, LLMConfig, SandboxConfig
from azm_ai.core.logger import azm_ai_logger as logger
from azm_ai.core.main import create_runtime, run_controller
from azm_ai.events.action import CmdRunAction, MessageAction
...
async def resolve_issue(
    ...
    if repo_instruction is None:
        # Check for .azm_ai_instructions file in the workspace directory
        azm_ai_instructions_path = os.path.join(repo_dir, '.azm_ai_instructions')
        if os.path.exists(azm_ai_instructions_path):
            with open(azm_ai_instructions_path, 'r') as f:  # noqa: ASYNC101
...
def main() -> None:
    ...
    if runtime_container_image is None and not my_args.is_experimental:
        runtime_container_image = (
            f'ghcr.io/all-hands-ai/runtime:{azm_ai.__version__}-nikolaik'
        )
