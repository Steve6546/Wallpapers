...
from azm_ai.resolver.interfaces.issue import Issue
from azm_ai.resolver.interfaces.issue_definitions import ServiceContextIssue
from azm_ai.resolver.io_utils import (
    load_all_resolver_outputs,
    load_single_resolver_output,
)
from azm_ai.resolver.patching import apply_diff, parse_patch
from azm_ai.resolver.resolver_output import ResolverOutput
from azm_ai.resolver.utils import (
    Platform,
    identify_token,
)
...
def make_commit(repo_dir: str, issue: Issue, issue_type: str) -> None:
    ...
    if not result.stdout.strip():
        ...
        subprocess.run(
            f'git -C {repo_dir} config user.name "azm_ai" && '
            f'git -C {repo_dir} config user.email "azm-ai@steve6546.com" && '
            f'git -C {repo_dir} config alias.git "git --no-pager"',
            shell=True,
            check=True,
        )
        logger.info('Git user configured as azm_ai')
    ...
    if not status_result.stdout.strip():
        ...
        raise RuntimeError('ERROR: AZM AI failed to make code changes.')
...
def send_pull_request(
    ...
    base_branch_name = f'azm-ai-fix-issue-{issue.number}'
    ...
    if additional_message:
        pr_body += f'\n\n{additional_message}'
    pr_body += '\n\nAutomatic fix generated by [AZM AI](https://github.com/All-Hands-AI/AZM AI/) 🙌'
...
def update_existing_pull_request(
    ...
    if not comment_message and additional_message:
        try:
            ...
            if explanations:
                comment_message = (
                    'AZM AI made the following changes to resolve the issues:\n\n'
                )
            ...
            comment_message = f'A new AZM AI update is available, but failed to parse or summarize the changes:\n{additional_message}'
