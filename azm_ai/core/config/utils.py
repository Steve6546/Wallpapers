...
def load_from_toml(cfg: AppConfig, toml_file: str = 'config.toml') -> None:
    ...
    # set LLMSummarizingCondenserConfig as default
        ...
        from azm_ai.core.config.condenser_config import LLMSummarizingCondenserConfig
    ...
    for key in toml_config:
        if key.lower() not in known_sections:
            logger.azm_ai_logger.warning(f'Unknown section [{key}] in {toml_file}')
...
def get_agent_config_arg(
    ...
    logger.azm_ai_logger.debug(f'Loading agent config from {agent_config_arg}')
    ...
    try:
        ...
        logger.azm_ai_logger.error(f'Config file not found: {e}')
        return None
    except toml.TomlDecodeError as e:
        logger.azm_ai_logger.error(
    ...
    logger.azm_ai_logger.debug(f'Loading from toml failed for {agent_config_arg}')
    return None
...
def get_llm_config_arg(
    ...
    logger.azm_ai_logger.debug(f'Loading llm config from {llm_config_arg}')
    ...
    logger.azm_ai_logger.debug(f'Loading from toml failed for {llm_config_arg}')
    return None
...
def parse_arguments() -> argparse.Namespace:
    ...
    if args.version:
        print(f'AZM AI version: {__version__}')
        sys.exit(0)
