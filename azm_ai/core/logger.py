...
def get_file_handler(
    ...
    file_name = f'azm-ai_{timestamp}.log'
...
azm_ai_logger = logging.getLogger('azm-ai')
current_log_level = logging.INFO

if LOG_LEVEL in logging.getLevelNamesMapping():
    current_log_level = logging.getLevelNamesMapping()[LOG_LEVEL]
azm_ai_logger.setLevel(current_log_level)

if DEBUG:
    azm_ai_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    LOG_TO_FILE = True
    azm_ai_logger.debug('DEBUG mode enabled.')

if LOG_JSON:
    azm_ai_logger.addHandler(json_log_handler(current_log_level))
else:
    azm_ai_logger.addHandler(get_console_handler(current_log_level))

azm_ai_logger.addFilter(SensitiveDataFilter(azm_ai_logger.name))
azm_ai_logger.propagate = False
azm_ai_logger.debug('Logging initialized')

LOG_DIR = os.path.join(
    # parent dir of azm_ai/core (i.e., root of the repo)
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    'logs',
)

if LOG_TO_FILE:
    azm_ai_logger.addHandler(
        get_file_handler(LOG_DIR, current_log_level)
    )  # default log to project root
    azm_ai_logger.debug(f'Logging to file in: {LOG_DIR}')
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        super().emit(record)
        self.stream.close()
        azm_ai_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
...
class AZMAILoggerAdapter(logging.LoggerAdapter):
    extra: dict

    def __init__(
        self, logger: logging.Logger = azm_ai_logger, extra: dict | None = None
    ) -> None:
        self.logger = logger
        self.extra = extra or {}
