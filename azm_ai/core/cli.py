...
import azm_ai.agenthub  # noqa F401 (we import this to get the agents registered)
from azm_ai import __version__
...
from azm_ai.core.loop import run_agent_until_done
from azm_ai.core.schema import AgentState
from azm_ai.core.setup import (
    create_agent,
    create_controller,
    create_memory,
    create_runtime,
...
from azm_ai.events import EventSource, EventStreamSubscriber
...
from azm_ai.io import read_task
from azm_ai.llm.metrics import Metrics
from azm_ai.mcp import fetch_mcp_tools_from_config
from azm_ai.microagent.microagent import BaseMicroagent
...
Please explore this repository. Create the file .azm_ai/microagents/repo.md with:
...
def display_command_output(output: str):
    ...
    formatted_lines = []
    for line in lines:
        if line.startswith('[Python Interpreter') or line.startswith('azm_ai@'):
...
def display_help(style=DEFAULT_STYLE):
    ...
    print_formatted_text('Some tips to get the most out of AZM AI:')
    print_formatted_text(
...
async def init_repository(current_dir: str) -> bool:
    repo_file_path = Path(current_dir) / '.azm_ai' / 'microagents' / 'repo.md'
    init_repo = False
...
def manage_azm_ai_file(folder_path=None, add_to_trusted=False):
    azm_ai_file = Path.home() / '.azm_ai.toml'
    default_content: dict = {'trusted_dirs': []}

    if not azm_ai_file.exists():
        with open(azm_ai_file, 'w') as f:
            toml.dump(default_content, f)

    if folder_path:
        with open(azm_ai_file, 'r') as f:
            try:
                config = toml.load(f)
            except Exception:
...
def check_folder_security_agreement(current_dir):
    is_trusted = manage_azm_ai_file(current_dir)
    ...
    if not is_trusted:
        ...
        if confirm:
            manage_azm_ai_file(current_dir, add_to_trusted=True)
...
async def main(loop: asyncio.AbstractEventLoop):
    ...
    # Show AZM AI banner and session ID
