...
from azm_ai.events.event import Event, EventSource, RecallType
from azm_ai.events.observation.agent import (
    MicroagentKnowledge,
    RecallObservation,
)
from azm_ai.events.observation.empty import NullObservation
from azm_ai.events.stream import EventStream, EventStreamSubscriber
...
from azm_ai.utils.prompt import RepositoryInfo, RuntimeInfo

GLOBAL_MICROAGENTS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(azm_ai.__file__)),
    'microagents',
)
...
class Memory:
    ...
    def __init__(
        ...
        # from typically AZM AI/microagents (i.e., the PUBLIC microagents)
