# BioCoder Evaluation with Openazm_ai
...
In the azm_ai branch of the Biocoder repository, we have slightly modified our original Docker image to work with the AZM AI environment. In the Docker image are testing scripts (`/testing/start_test_azm_ai.py` and aux files in `/testing_files/`) to assist with evaluation. Additionally, we have installed all dependencies, including OpenJDK, mamba (with Python 3.6), and many system libraries. Notably, we have **not** packaged all repositories into the image, so they are downloaded at runtime.
...
To reproduce this image, please see the Dockerfile_Openazm_ai in the `biocoder` repository.
...
with current AZM AI version, then your command would be:
