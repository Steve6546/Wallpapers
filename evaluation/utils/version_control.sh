checkout_eval_branch() {
    ...
    echo "Start to checkout azm_ai version to $COMMIT_HASH, but keep current evaluation harness"
    ...
    echo "Check out AZM AI to version: $COMMIT_HASH"
...
get_azm_ai_version() {
    # IMPORTANT: Because Agent's prompt changes fairly often in the rapidly evolving codebase of AZM AI
    # We need to track the version of Agent in the evaluation to make sure results are comparable
    AZM_AI_VERSION=v$(poetry run python -c "from azm_ai import get_version; print(get_version())")
}
