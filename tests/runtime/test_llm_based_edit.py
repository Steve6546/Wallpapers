...
from azm_ai.events.action import FileEditAction, FileReadAction
from azm_ai.events.observation import FileEditObservation
...
def test_edit_from_scratch(temp_dir, runtime_cls, run_as_azm_ai):
...
def test_edit(temp_dir, runtime_cls, run_as_azm_ai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_azm_ai)
    try:
        action = FileEditAction(
            content=ORGINAL,
...
def test_edit_long_file(temp_dir, runtime_cls, run_as_azm_ai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_azm_ai)
    try:
        action = FileEditAction(
