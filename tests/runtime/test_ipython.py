...
def test_simple_cmd_ipython_and_fileop(temp_dir, runtime_cls, run_as_azm_ai):
    ...
    assert obs.content.strip() == (
        ...
        '[Jupyter Python interpreter: /azm_ai/poetry/azm_ai-ai-5O4_aCHf-py3.12/bin/python]'
    )
...
def test_ipython_multi_user(temp_dir, runtime_cls, run_as_azm_ai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_azm_ai)
    ...
    assert obs.exit_code == 0
    if run_as_azm_ai:
        # -rw-r--r-- 1 azm_ai root 13 Jul 28 03:53 test.txt
        assert 'azm_ai' in obs.content.split('\r\n')[0]
    else:
...
def test_ipython_package_install(temp_dir, runtime_cls, run_as_azm_ai):
...
def test_ipython_file_editor_permissions_as_azm_ai(temp_dir, runtime_cls):
    """Test file editor permission behavior when running as different users."""
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_azm_ai=True)
    ...
    # Try to view the file as azm_ai user - should fail with permission denied
    ...
    # Try to edit the file as azm_ai user - should fail with permission denied
    ...
    # Try to use file editor in azm_ai sandbox directory - should work
    test_code = """
# Create file
