...
from azm_ai.server.data_models.conversation_info_result_set import (
...
from azm_ai.storage.data_models.conversation_status import ConversationStatus
from azm_ai.storage.locations import get_conversation_metadata_filename
...
def _patch_store():
    ...
    with patch(
        'azm_ai.storage.conversation.file_conversation_store.get_file_store',
        MagicMock(return_value=file_store),
    ):
        with patch(
            'azm_ai.server.routes.manage_conversations.conversation_manager.file_store',
            file_store,
        ):
            yield
...
async def test_search_conversations():
    with _patch_store():
        with patch(
            'azm_ai.server.routes.manage_conversations.config'
        ) as mock_config:
            mock_config.conversation_max_age_seconds = 864000  # 10 days
            with patch(
                'azm_ai.server.routes.manage_conversations.conversation_manager'
            ) as mock_manager:
                ...
                with patch(
                    'azm_ai.server.routes.manage_conversations.datetime'
                ) as mock_datetime:
