...
from azm_ai.resolver.interfaces.github import GithubIssueHandler, GithubPRHandler
from azm_ai.resolver.interfaces.issue import Issue, ReviewThread
from azm_ai.resolver.interfaces.issue_definitions import (
    ServiceContextIssue,
    ServiceContextPR,
)
from azm_ai.resolver.resolve_issue import (
    complete_runtime,
    initialize_runtime,
    process_issue,
)
from azm_ai.resolver.resolver_output import ResolverOutput
...
async def test_resolve_issue_no_issues_found():
    from azm_ai.resolver.resolve_issue import resolve_issue
    ...
    with patch(
        'azm_ai.resolver.resolve_issue.issue_handler_factory',
...
async def test_process_issue(mock_output_dir, mock_prompt_template):
    ...
    for test_case in test_cases:
        ...
        with (
            patch(
                'azm_ai.resolver.resolve_issue.create_runtime', mock_create_runtime
            ),
            patch(
                'azm_ai.resolver.resolve_issue.initialize_runtime',
                mock_initialize_runtime,
            ),
            patch(
                'azm_ai.resolver.resolve_issue.run_controller', mock_run_controller
            ),
            patch(
                'azm_ai.resolver.resolve_issue.complete_runtime',
                mock_complete_runtime,
            ),
            patch('azm_ai.resolver.resolve_issue.logger'),
        ):
            # Call the function
...
def test_file_instruction():
    ...
    # load prompt from azm_ai/resolver/prompts/resolve/basic.jinja
    with open('azm_ai/resolver/prompts/resolve/basic.jinja', 'r') as f:
        prompt = f.read()
...
def test_file_instruction_with_repo_instruction():
    ...
    with open('azm_ai/resolver/prompts/resolve/basic.jinja', 'r') as f:
        prompt = f.read()
    # load repo instruction from azm_ai/resolver/prompts/repo_instructions/all-hands-ai___azm-ai-resolver.txt
    with open(
        'azm_ai/resolver/prompts/repo_instructions/all-hands-ai___azm-ai-resolver.txt',
        'r',
    ) as f:
...
This is a Python repo for azm-ai-resolver, a library that attempts to resolve github issues with the AI agent AZM AI.
