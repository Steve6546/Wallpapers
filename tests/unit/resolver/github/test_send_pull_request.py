...
from azm_ai.resolver.interfaces.github import GithubIssueHandler
from azm_ai.resolver.interfaces.issue import ReviewThread
from azm_ai.resolver.resolver_output import Issue, ResolverOutput
from azm_ai.resolver.send_pull_request import (
    apply_patch,
    initialize_repo,
...
from azm_ai.resolver.utils import Platform
...
@patch('azm_ai.resolver.interfaces.github.GithubIssueHandler.reply_to_comment')
@patch('httpx.post')
@patch('subprocess.run')
@patch('azm_ai.resolver.send_pull_request.LLM')
...
def test_send_pull_request(
    ...
    assert checkout_call == call(
        ['git', '-C', repo_path, 'checkout', '-b', 'azm-ai-fix-issue-42'],
        capture_output=True,
        text=True,
    )
    ...
    if pr_type == 'branch':
        assert (
            result
            == 'https://github.com/test-owner/test-repo/compare/azm-ai-fix-issue-42?expand=1'
        )
        ...
        assert post_data['head'] == 'azm-ai-fix-issue-42'
...
def test_send_pull_request_target_branch_with_fork(
    ...
    assert (
        post_data['head'] == 'fork-owner:azm-ai-fix-issue-42'
    ...
    mock_response.json.return_value = {
        'data': {
            'addPullRequestReviewThreadReply': {
                'comment': {
                    'id': 'test_reply_id',
                    'body': 'AZM AI fix success summary\n\n\nThis is a test reply.',
...
@patch('azm_ai.resolver.send_pull_request.initialize_repo')
@patch('azm_ai.resolver.send_pull_request.apply_patch')
@patch('azm_ai.resolver.send_pull_request.update_existing_pull_request')
@patch('azm_ai.resolver.send_pull_request.make_commit')
...
@patch('azm_ai.resolver.send_pull_request.initialize_repo')
@patch('azm_ai.resolver.send_pull_request.apply_patch')
@patch('azm_ai.resolver.send_pull_request.send_pull_request')
@patch('azm_ai.resolver.send_pull_request.make_commit')
...
@patch('azm_ai.resolver.send_pull_request.load_all_resolver_outputs')
@patch('azm_ai.resolver.send_pull_request.process_single_issue')
...
def test_send_pull_request_branch_naming(
    ...
    assert checkout_call == call(
        ['git', '-C', repo_path, 'checkout', '-b', 'azm-ai-fix-issue-42-try3'],
        capture_output=True,
        text=True,
    )
    assert push_call == call(
    ...
    assert (
        result
        == 'https://github.com/test-owner/test-repo/compare/azm-ai-fix-issue-42-try3?expand=1'
    )


@patch('azm_ai.resolver.send_pull_request.argparse.ArgumentParser')
@patch('azm_ai.resolver.send_pull_request.process_all_successful_issues')
@patch('azm_ai.resolver.send_pull_request.process_single_issue')
@patch('azm_ai.resolver.send_pull_request.load_single_resolver_output')
@patch('azm_ai.resolver.send_pull_request.identify_token')
...
def test_main(
    ...
    from azm_ai.resolver.send_pull_request import main
...
def test_make_commit_no_changes(mock_subprocess_run):
    ...
    with pytest.raises(
        RuntimeError, match='ERROR: AZM AI failed to make code changes.'
    ):
