...
def _check_source_code_in_dir(temp_dir):
    ...
    # The source code should only include the `azm_ai` folder,
    ...
    assert os.path.exists(os.path.join(code_dir, 'azm_ai'))
    assert os.path.isdir(os.path.join(code_dir, 'azm_ai'))
    ...
    assert _pyproject_version == version('azm_ai-ai')
...
def test_generate_dockerfile_build_from_scratch():
    ...
    assert 'COPY ./code/azm_ai /azm_ai/code/azm_ai' in dockerfile_content
    assert (
        '/azm_ai/micromamba/bin/micromamba run -n azm_ai poetry install'
...
def live_docker_image():
    ...
    RUN echo "Hello, AZM AI!" > /azm_ai/hello.txt
    """
