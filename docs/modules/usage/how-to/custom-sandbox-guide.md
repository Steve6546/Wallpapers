...
The default AZM AI sandbox (`python-nodejs:python3.12-nodejs22`
...
For example, if you want AZM AI to have `ruby` installed, you could create a `Dockerfile` with the following content:
...
When running AZM AI using [the docker command](/modules/usage/installation#start-the-app), replace
...
First, ensure you can run AZM AI by following the instructions in [Development.md](https://github.com/All-Hands-AI/AZM AI/blob/main/Development.md).

### Specify the Base Sandbox Image

In the `config.toml` file within the AZM AI directory, set the `base_container_image` to the image you want to use.
...
Run AZM AI by running ```make run``` in the top level directory.
