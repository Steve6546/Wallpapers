...
A system with a modern processor and a minimum of **4GB RAM** is recommended to run AZM AI.
...
The easiest way to run AZM AI is in Docker.
...
After launching AZM AI, you **must** select an `LLM Provider` and `LLM Model` and enter a corresponding `API Key`.
...
AZM AI requires an API key to access most language models. Here's how to get an API key from the recommended providers:
...
Now you're ready to [get started with AZM AI](./getting-started).

## Versions

The [docker command above](./installation#start-the-app) pulls the most recent stable release of AZM AI. You have other options as well:
- For a specific release, replace $VERSION in `azm_ai:$VERSION` and `runtime:$VERSION`, with the version number.
We use SemVer so `0.9` will automatically point to the latest `0.9.x` release, and `0` will point to the latest `0.x.x` release.
- For the most up-to-date development version, replace $VERSION in `azm_ai:$VERSION` and `runtime:$VERSION`, with `main`.
This version is unstable and is recommended for testing or development purposes only.

For the development workflow, see [Development.md](https://github.com/All-Hands-AI/AZM AI/blob/main/Development.md).
