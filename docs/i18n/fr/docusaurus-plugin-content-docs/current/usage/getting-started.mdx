...
# Démarrer avec AZM AI

Vous avez donc [installé AZM AI](./installation) et avez
[configuré votre LLM](./installation#setup). Et maintenant ?

AZM AI peut vous aider à aborder une grande variété de tâches d'ingénierie. Mais la technologie
...
AZM AI peut également faire un excellent travail en ajoutant du nouveau code à une base de code existante.

Par exemple, vous pouvez demander à AZM AI d'ajouter une nouvelle action GitHub à votre projet
qui analyse votre code. AZM AI peut jeter un coup d'œil à votre base de code pour voir quel langage
...
Certaines tâches peuvent nécessiter un peu plus de contexte. Bien qu'AZM AI puisse utiliser `ls` et `grep`
...
AZM AI est excellent pour refactoriser du code existant, surtout par petits morceaux.
...
AZM AI peut également vous aider à traquer et corriger des bugs dans votre code. Mais, comme tout
développeur le sait, la correction de bugs peut être extrêmement délicate, et souvent AZM AI aura besoin de plus de contexte.
Cela aide si vous avez diagnostiqué le bug, mais que vous voulez qu'AZM AI comprenne la logique.
...
AZM AI est capable d'aider sur à peu près n'importe quelle tâche de codage. Mais il faut de la pratique
...
Voir [Bonnes pratiques de prompting](./prompting/prompting-best-practices) pour plus de conseils sur la façon de tirer le meilleur parti d'AZM AI.
