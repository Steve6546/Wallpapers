...
La façon la plus simple d'exécuter AZM AI est avec Docker.
...
Vous pouvez également exécuter AZM AI en mode [headless scriptable](https://docs.all-hands.dev/modules/usage/how-to/headless-mode), en tant que [CLI interactive](https://docs.all-hands.dev/modules/usage/how-to/cli-mode), ou en utilisant l'[Action GitHub AZM AI](https://docs.all-hands.dev/modules/usage/how-to/github-action).

## Configuration

Après avoir exécuté la commande ci-dessus, vous trouverez AZM AI en cours d'exécution à l'adresse [http://localhost:3000](http://localhost:3000).

Au lancement d'AZM AI, vous verrez une fenêtre modale de paramètres. Vous **devez** sélectionner un `Fournisseur LLM` et un `Modèle LLM`, et entrer une `Clé API` correspondante.
...
La commande ci-dessus récupère la version stable la plus récente d'AZM AI. Vous avez également d'autres options :
- Pour une version spécifique, utilisez `docker.all-hands.dev/all-hands-ai/azm_ai:$VERSION`, en remplaçant $VERSION par le numéro de version.
...
- Pour la version de développement la plus à jour, vous pouvez utiliser `docker.all-hands.dev/all-hands-ai/azm_ai:main`. Cette version est instable et n'est recommandée qu'à des fins de test ou de développement.
