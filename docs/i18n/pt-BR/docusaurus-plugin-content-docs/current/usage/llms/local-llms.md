...
## Run AZM AI with Docker

### Start AZM AI
Use the instructions [here](../getting-started) to start AZM AI using Docker.
...
When running `azm_ai`, you'll need to set the following in the AZM AI UI through the Settings:
...
## Run AZM AI in Development Mode
...
Done! Now you can start AZM AI by: `make run`. You now should be able to connect to `http://localhost:3000/`

### Configure the Web Application

In the AZM AI UI, click on the Settings wheel in the bottom-left corner.
...
The default configuration for ollama in WSL only serves localhost. This means you can't reach it from a docker container. eg. it wont work with AZM AI. First let's test that ollama is running correctly.
...
```bash
docker ps # get list of running docker containers, for most accurate test choose the AZM AI sandbox container.
...
Done! Now you can start AZM AI by: `make run` without Docker. You now should be able to connect to `http://localhost:3000/`
